...
begin
  try
   try
      ...
      begin
        mFornecedores := Fornecedores(pObj);
        mFornecedores.SetCodigo(umDM.qFornecedores.FieldByName('CODFORNECEDOR').value);
        ...
        mFornecedores.SetSite(umDM.qFornecedores.FieldByName('SITE').AsString);
...
begin
    try
      ...
      begin
        ...
        mSql := 'insert into Fornecedores (cnpj, razaosocial, nomefantasia, telefone, email, '+
        ...
        begin
          mSql := 'update Fornecedores set cnpj = :cnpj, razaosocial = :razaosocial, '+
                  'nomefantasia = :nomefantasia, telefone = :telefone, email = :email, '+
                  'site = :site, rg_ie = :rg_ie, endereco = :endereco, numero = :numero, '+
                  'bairro = :bairro, cep = :cep, complemento = :complemento, codcidade = :codcidade';
...
begin
  try
    mFornecedores := Fornecedores(pObj);
    mSql := 'delete from fornecedores where codFornecedor = '+inttostr(mFornecedores.getCodigo);
    umDm.Trans.StartTransaction;
    umDm.qFornecedores.Active:= false;
    //umDm.qFornecedores.SQL.Clear;
    //umDM.qFornecedores.ExecSQL(mSql);
    umDm.Trans.Commit;
    result := '';
  except on e: Exception do
    begin
